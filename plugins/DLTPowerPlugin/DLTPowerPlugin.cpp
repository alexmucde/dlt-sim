...
void DLTPowerPlugin::checkPortName()
{
...
void DLTPowerPlugin::clearSettings()
{
    type = 0;
    powerName = "Power";
    substractCurrent = 0;
...
void DLTPowerPlugin::settings(QWidget *parent)
{
    DLTPowerSettingsDialog dlg(parent);
...
void DLTPowerPlugin::readSettings(const QString &filename,const int instance)
{
    ...
    while (!xml.atEnd())
    {
          ...
          if(xml.isStartElement())
          {
              ...
              {
                  if( xml.readElementText() == QString(DLT_POWER_PLUGIN_NAME))
                    isDLTCan = true;
              }
          }
...
void DLTPowerPlugin::writeSettings(QXmlStreamWriter &xml,const int instance) const
{
    // Plugin type and version
    xml.writeTextElement("Name",DLT_POWER_PLUGIN_NAME);
    xml.writeTextElement("Version",DLT_POWER_PLUGIN_VERSION);
...
void DLTPowerPlugin::start(DLTMiniServer *newDltMiniServer)
{
    ...
    if(serialPort.open(QIODevice::ReadWrite)==true)
    {
        ...
        emit updateStatus();
        qDebug() << "DLTPowerPlugin: started" << interface;
     }
    else
    {
        // open failed

        qDebug() << "DLTPowerPlugin: Failed to open interface" << interface;
        pluginStatus = "error";
        emit updateStatus();
    }
...
void DLTPowerPlugin::stop()
{
    // stop communication
    pluginStatus = "stopped";
    emit updateStatus();
    qDebug() << "DLTPowerPlugin: stopped" << interface;
...
void DLTPowerPlugin::show(QMdiArea *mdiArea)
{
    if(window == 0)
        window = new QMdiSubWindow;
    if(form == 0)
        form = new DLTPowerForm(this,window);
...
void DLTPowerPlugin::update()
{
    if(form)
        form->update();
}

void DLTPowerPlugin::hide()
{
    if(window)
        window->hide();
    delete window;
        ...
        for(int num=0;num<data.length();num++)
        {
            switch(data[num]&0xf0)
            {
                ...
                case 0xe0:
                    data.clear();
                    rawData+=data[num];
                    //qDebug() << "DLTPowerPlugin: Raw Data " << rawData.toHex();
                    calculateValue();
                    if(form)
                    ...
                    watchDogCounter++;
                    //qDebug() << "DLTPowerPlugin: Value received" << interface << value << unit;
                    break;
                default:
                    rawData+=data[num];
                    break;
    ...
    else if(type==1) // Mason HCS-3302 USB
    {
        ...
        while(serialPort.bytesAvailable())
        {
            ...
            while(pos!=-1)
            {
                ...
                //qDebug() << "DLTPowerPlugin: readLine" << line;
...
//                    valueMultimeter(QString("%1").arg(current),"A");
    ...
    else if(type==2) // WemosD1MiniPower
    {
        ...
        while(serialPort.bytesAvailable())
        {
            ...
            {
                // line is not empty
                //qDebug() << "DLTPowerPlugin: readLine" << data;
...
void DLTPowerPlugin::timeoutRequest()
{
    if(type==1) // Mason HCS-3302 USB
    {
...
float DLTPowerPlugin::getSubstractCurrent() const
{
    return substractCurrent;
}

void DLTPowerPlugin::setSubstractCurrent(float newSubstractCurrent)
{
    substractCurrent = newSubstractCurrent;
}

void DLTPowerPlugin::timeout()
{
    // watchdog timeout
    ...
    else
    {
        ...
        qDebug() << "DLTPowerPlugin: Watchdog expired try to reconnect" ;
        ...
        if(serialPort.open(QIODevice::ReadWrite)==true)
        {
            ...
            emit updateStatus();
            qDebug() << "DLTPowerPlugin: reconnect" << interface;
        }
        else
        {
            // retry failed
...
void DLTPowerPlugin::calculateValue()
{
...
int DLTPowerPlugin::calculateNumber(unsigned char a,unsigned char b)
{
    if(a==0x7 && b==0x0d)
        return 0;
...
void DLTPowerPlugin::on()
{
    if(type==1) // Mason HCS-3302 USB
    {
        serialPort.write("SOUT0\r");

        qDebug() << "DLTPowerPlugin: power on";
        if(dltMiniServer)
            dltMiniServer->sendValue2(dltAppId,dltCtxId,powerName,"On");
    }
}

void DLTPowerPlugin::off()
{
    if(type==1) // Mason HCS-3302 USB
    {
        serialPort.write("SOUT1\r");

        qDebug() << "DLTPowerPlugin: power off";
        if(dltMiniServer)
            dltMiniServer->sendValue2(dltAppId,dltCtxId,powerName,"Off");
    }
}

void DLTPowerPlugin::setVoltage(float value)
{
    if(type==1) // Mason HCS-3302 USB
    {
        QString text;
        ...
        //qDebug() << "DLTPowerPlugin: setVoltage" << text;
    }
}

void DLTPowerPlugin::backupGeometry()
{
    QSettings settings;
    settings.setValue("dlt_power_plugin_geometry", window->saveGeometry());
}

void DLTPowerPlugin::restoreGeometry()
{
    QSettings settings;
    window->restoreGeometry(settings.value("dlt_power_plugin_geometry").toByteArray());
}

void DLTPowerPlugin::injection(QString data)
{
    if(form)
        form->injection(data);
}
